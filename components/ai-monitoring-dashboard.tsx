...
import { fetchAIFlags, fetchAgentMetrics } from "../lib/services/aiMonitoringApi";
...
export function AIMonitoringDashboard() {
  const { data: flags, loading: flagsLoading, error: flagsError } = useMockApi(fetchAIFlags);
  const { data: agentMetrics, loading: metricsLoading, error: metricsError } = useMockApi(fetchAgentMetrics);
  ...
  if (flagsLoading || metricsLoading) return <div>Loading...</div>;
  if (flagsError || metricsError) return <div>Error loading AI monitoring data.</div>;
